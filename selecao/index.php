    ...
    <body>
		<form action="" method="POST">        
			...
			<select name="id_categoria" id="id_categoria">
				...
				<?php
					$result_cat = "SELECT * FROM turma ORDER BY anoLectivo";
					$resultado_cat = mysqli_query($conn, $result_cat);
					while($row_cat = mysqli_fetch_assoc($resultado_cat) ) {
						echo '<option value="'.$row_cat['id'].'">'.$row_cat['anoLectivo'].'</option>';
					}
				?>
			</select><br><br>
		...
		<?php
			if($_SERVER['REQUEST_METHOD']=='POST'){
				...
				$result_pesquisar = "SELECT anoLectivo FROM turma WHERE turma = '$turma' AND subcategoria_id = '$id_sub_categoria'";
				
				$resultado_pesquisar = mysqli_query($conn, $result_pesquisar);
				
				while ($row_pesquisar = mysqli_fetch_assoc($resultado_pesquisar)){
					echo "Empresa: ".$row_pesquisar['anoLectivo'] . "<br>";
				}
